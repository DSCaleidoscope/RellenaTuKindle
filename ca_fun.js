...
function endLoad() {
  g('mbody').style.backgroundColor = "#71ff71";
  g('mainScreen').style.visibility = "initial";
  g('loader').style.visibility = "hidden";
  g('mainScreen').style.background = "#ffffff61";
  g('mainScreen').style.borderRadius = "35px";
}
...
function collapseBook(e) {
  ...
  let i = 0; //here is 0 because we only have one div (cover)
  ...
  childs[i].style.width = defWidth + "px";
  if (!childs[0].classList.contains("c2")) {
    //if we have c2 class, we shall skip all this
    if (Bcollapsed[e.id] == true) {
      ...
      for (; i < x; i++) {
        ...
        childs[i].style.height = defHeight + "px";
  ...
  //animations!
  if (!childs[0].classList.contains("c1")) {
    //first click. Do nothing
    childs[0].classList.add("c1");
  } else if (!childs[0].classList.contains("c2")) { 
    //second click. Animation
    childs[0].classList.add("c2");
  }
}
...
async function fill() {
  ...
  let isDone = localStorage.getItem('rtk_ca_done');
  let dv = new Date();
  ...
  if (isDone == doneValue) {
    ...
    bookList = JSON.parse(localStorage.getItem('rtk_ca_books'));
    bookMax = localStorage.getItem('rtk_ca_count');
    ...
    if (cm == 11) {
      ...
      let bp = filePromise("book_ca" + cd + ".json").then(
      ...
      localStorage.setItem('rtk_ca_books', JSON.stringify(bookList));
      localStorage.setItem('rtk_ca_count', bookMax);
      localStorage.setItem('rtk_ca_done', doneValue);
    }
  }
